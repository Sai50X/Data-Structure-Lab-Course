...
public class World {
    ...
    * @param currentX - the current x positi on
   ...
   private int light(int x, int y, int currentX, int currentY, double r)  {
      // ***** <YOUR CODE GOES HERE> *****
      //Replace this code with a recursive method that only lights up tiles within the radius
      double dist = Math.sqrt(Math.pow(Math.abs(currentX-x), 2) + Math.pow(Math.abs(currentY-y),2));
      ...
      return light(x,y,currentX-1, currentY,r) + light(x,y,currentX+1, currentY,r) + light(x,y,currentX, currentY-1,r) + light(x,y,currentX, currentY+1,r) + 1;
